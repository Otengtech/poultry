...
import { generateOrderId, generateOrderNumber, getDeviceId } from "../utils/orderUtils";
...
const CartPage = () => {
  ...
  try {
    ...
    const orderDataForBackend = {
      // Don't send id/orderNumber - let backend generate these
      customerInfo: {
      ...
      // Don't send createdAt/updatedAt - backend will set these
      // Don't send __v - backend will handle
    };

    console.log("Sending to backend:", orderDataForBackend);
    ...
    if (API_URL) {
      try {
        const response = await axios.post(`${API_URL}/order`, orderDataForBackend, {
          headers: {
        ...
        if (response.data.success) {
          // Generate local order data with backend response
          const orderId = generateOrderId();
          const orderNumber = response.data.order?.orderNumber || generateOrderNumber();
          const orderTimestamp = new Date().toISOString();
          
          // Create local order data with ALL product details
          const orderDataForLocal = {
            id: orderId,
            ...
            items: cartItems.map((item, index) => ({
              _id: item._id || `PROD${(index + 1).toString().padStart(3, "0")}`,
              productId: item._id || `PROD${(index + 1).toString().padStart(3, "0")}`,
              name: item.name,
              price: parseFloat(item.price),
              quantity: item.quantity,
            ...
            deliveryFee: userInfo.deliveryType === 'delivery' ? 10 : 0,
            deviceId: getDeviceId(),
            deletionAllowed: false,
            deletionAllowedAt: new Date(Date.now() + 30 * 60 * 1000).toISOString(),
            serverId: response.data.order?._id, // Store backend ID if available
          };

          // Save to localStorage
          saveOrderToLocalStorage(orderDataForLocal);
        ...
        if (error.code === "ERR_NETWORK" || !error.response) {
          // Network error - fallback to local storage only
          toast.warning("Backend not connected. Saving order locally...");
          saveOrderLocally();
        } else if (error.response?.status === 400) {
          // Bad Request - show specific error
          const errorMsg = error.response.data?.message || "Invalid order data";
...
const sendOrderToWhatsApp = (orderData) => {
  const phoneNumber = "233593957373"; // replace with your number
...
🚚 Delivery: ${orderData.customerInfo.deliveryType}
📍 Address: ${orderData.customerInfo.address || "N/A"}

🧾 Items:
${orderData.items.map(item =>
  `- ${item.name} x${item.quantity} (GHS ${item.totalPrice})`
).join("\n")}
...
const saveOrderLocally = () => {
  ...
  // Save to localStorage
  saveOrderToLocalStorage(orderDataForLocal);
  sendOrderToWhatsApp(orderDataForLocal);

  // Clear cart
  updateCart([]);

  toast.success(`Order ${orderNumber} saved locally!`);
  setShowCheckoutModal(false);
  setUserInfo({
    name: "",
    phone: "",
          ...
          <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    {userInfo.deliveryType === "delivery" ? "Delivery Address" : "Pickup Location"}
                  </label>
                  <input
                    type="text"
                    name="address"
                    ...
                    onChange={handleInputChange}
                    placeholder={userInfo.deliveryType === "delivery" ? "Enter your full address" : "Our farm location"}
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                    disabled={isSubmitting}
                  />
                  {userInfo.deliveryType === "pickup" && (
              ...
              <div className="mt-6 p-4 bg-blue-50 rounded-lg">
                <p className="text-sm text-gray-600">
                  <strong>Note:</strong> Your order will be stored locally on this device. 
                  Orders are device-specific and cannot be accessed from other devices for privacy.
                </p>
              </div>
            </div>
          </div>
